...
SEARCH_RG_QUEUE_MAXSIZE = 1024
# 导出写入队列最大项数（施加背压）
EXPORT_WRITE_QUEUE_MAXSIZE = 512
# 导出文件缓冲区大小（字节，越大写盘吞吐越高）
EXPORT_WRITE_BUFFER_SIZE = 256 * 1024  # 256KB
# 写入线程批量写入的最大项数（减少频繁系统调用）
EXPORT_WRITER_BATCH_MAX_ITEMS = 64
# 写入线程批量写入的目标字符数（近似控制批大小）
EXPORT_WRITER_BATCH_MAX_BYTES = 32 * 1024  # 32KB（按字符近似）
# 周期性刷新间隔（毫秒，0 表示仅在关闭时刷新）
EXPORT_WRITER_FLUSH_INTERVAL_MS = 100	# 每100ms强制刷新一次
